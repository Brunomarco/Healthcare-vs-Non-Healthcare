    ...
    try:
        ...
        for sheet_name in all_sheet_names:
            try:
                ...
                # Store EMEA-only data (for gross metrics)
        ...
        # Combine EMEA-only data (no STATUS filter)
        ...
        # Categorize into healthcare and non-healthcare (for filtered data)
        ...
        # Categorize EMEA-only data for gross metrics
        ...
        # NEW: Create account classification dataframe for the new tab
        account_classification = pd.DataFrame()
        if not combined_raw_df.empty and 'ACCT NM' in combined_raw_df.columns:
            unique_accounts = combined_raw_df['ACCT NM'].dropna().unique()
            classifications = []
            
            for account in unique_accounts:
                # Check if account is in EMEA
                account_rows = combined_raw_df[combined_raw_df['ACCT NM'] == account]
                is_emea = False
        ...
        return healthcare_df, non_healthcare_df, stats, healthcare_gross_df, non_healthcare_gross_df, account_classification
...
with st.spinner("Processing ALL sheets and ALL rows from Excel file..."):
    healthcare_df, non_healthcare_df, stats, healthcare_gross_df, non_healthcare_gross_df, account_classification = read_and_combine_sheets(uploaded_file)
...
# Create tabs - ENHANCED with new Account Classification tab
tab1, tab2, tab3 = st.tabs(["🏥 Healthcare", "✈️ Non-Healthcare", "📋 Account Classification"])
...
# NEW TAB: Account Classification
with tab3:
    ...
    if not account_classification.empty:
        ...
        classification_filter = st.selectbox(
            ...
            options=["All", "Healthcare", "Non-Healthcare", "Not Used (Non-EMEA)"],
            index=0
        )
        ...
        with st.expander("Classification Logic"):
            ...
            **How accounts are classified:**
            ...
            2. **Healthcare**: EMEA accounts matching healthcare keywords or from specific sheets (e.g., AMS)
            3. **Non-Healthcare**: EMEA accounts not matching healthcare criteria
            
            **Note:** Classification is based on account name patterns and source sheet context.
            """)
